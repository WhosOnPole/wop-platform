...
export function PollDiscussionModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[80] flex items-start justify-center overflow-y-auto bg-black/80 px-4 pb-4"
      onClick={onClose}
    >
      <div
        className="mb-2 mt-[calc(4rem+env(safe-area-inset-top))] flex max-h-[calc(100dvh-5rem)] w-full max-w-4xl flex-col overflow-y-auto rounded-lg border border-white/10 bg-black/90 text-white shadow-2xl backdrop-blur-sm"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="shrink-0 p-6 pb-0">
          <div className="mb-6 flex items-start justify-between">
            <p className="min-w-0 flex-1 text-xl font-semibold text-white/70 font-sans tracking-wider [font-variant:all-small-caps;]">
              Poll Discussion
            </p>
            <button
              type="button"
