-- Create a function to get trending entities across all types
...
BEGIN
  ...
  SELECT 
    ...
    (COALESCE(comment_counts.comment_count, 0) * 3 + 
     ...
     COALESCE(fan_counts.fan_count, 0)) as trending_score
  ...
  SELECT 
    ...
    (COALESCE(team_comment_counts.comment_count, 0) * 3 + 
     ...
     COALESCE(team_fan_counts.fan_count, 0)) as trending_score
  ...
  SELECT 
    ...
    (COALESCE(track_comment_counts.comment_count, 0) * 3 + 
     COALESCE(track_fan_counts.fan_count, 0)) as trending_score
  ...
  SELECT 
    ...
    (COALESCE(tp_comment_counts.comment_count, 0) * 3) as trending_score
