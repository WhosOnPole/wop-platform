...
export const useTrackCommentLikes = (commentId?: string) => {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const { toast } = useToast();

  const { data: likesData, isLoading } = useQuery({
    queryKey: ['track-comment-likes', commentId],
    queryFn: async () => {
  ...
  const toggleLikeMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['track-comment-likes', commentId] });
    },
    onError: (error) => {
      toast({
        title: 'Error',
