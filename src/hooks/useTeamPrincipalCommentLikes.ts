...
export const useTeamPrincipalCommentLikes = (commentId?: string) => {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const { toast } = useToast();

  const { data: likesData, isLoading } = useQuery({
    queryKey: ['team-principal-comment-likes', commentId],
    queryFn: async () => {
  ...
  const toggleLikeMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['team-principal-comment-likes', commentId] });
    },
    onError: (error) => {
      toast({
        title: 'Error',
