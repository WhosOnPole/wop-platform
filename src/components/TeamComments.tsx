...
import { useTeamComments, useCreateTeamComment } from '@/hooks/useTeamComments';
...
const TeamComments = ({ teamId, teamName }: TeamCommentsProps) => {
  const { user } = useAuth();
  const { data: comments, isLoading } = useTeamComments(teamId);
      ...
      <CardContent className="space-y-6">
        {user ? (
          ...
          <div className="text-center py-6 text-muted-foreground">
            <p className="mb-4">Sign in to share your thoughts about {teamName}</p>
            <Link to="/auth">
        ...
        <div className="space-y-4">
          {comments && comments.length > 0 ? (
            comments.map((comment) => (
              <EnhancedCommentCard key={comment.id} comment={comment} type="team_comment" />
            ))
          ) : (
